...
def kleinbergMatrix(G):
	W = np.zeros((len(G), len(G)))
	A = np.zeros((len(G), len(G)))
	for x, src in enumerate(G.nodes()):
		for y, dst in enumerate(G.nodes()):
			if x == y:
				A[x][x] = (1. / (1. + len(G.neighbors(src))))
			if (src, dst) in G.edges():
				W[x][y] = (1 / (1. +  len(G.neighbors(src))))
				W[y][x] = (1 / (1. +  len(G.neighbors(dst))))
	return (W, A)
	
def kleinberg(data_files):
	savingPath = EXPERIMENT_PATH + OUTCOME_PATH + 'Kleinberg_Evolution/'
	f = open(checkDir(savingPath) + 'kleinberg_evolution.csv', 'w')
	for i in range(4):			
		G = nx.Graph()
		...
		for iteration, graph_file in enumerate(data_files):
			...
			(W, A) = kleinbergMatrix(G)
			
			ppt = pt.copy()
			pt = np.dot(W, pt) + np.dot(A, p0)
		#print 'ppt:', ppt
		#print 'pt:', pt
