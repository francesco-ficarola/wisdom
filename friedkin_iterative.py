...
def residualsModel(p, est, p1, p0, W):
	alpha = p
	pt = est.copy()
	for i in range(ITERATIONS_MODEL):
		pt = alpha * np.dot(W, pt) + (1-alpha) * p0
	
	err = np.linalg.norm(pt - p1)
	return err		
	
def friedkin(W, nodes, fileName):
	savingPath = EXPERIMENT_PATH + OUTCOME_PATH + 'Friedkin_Iterative/'
	...
	for i in range(4):
		...
		alpha0 = 0.5
		least_square = minimize(residualsModel, alpha0, args=(pt, p1, p0, W), method='SLSQP', bounds=[(0.1,0.999)])
		alpha = least_square['x'][0]
		#least_square = leastsq_bounds.leastsq_bounds(residualsModel, alpha0, [[0.1, 0.999]], args=(pt, p1, p0, W))
		#alpha = least_square[0][0]

		for j in range(ITERATIONS_MODEL):
			ppt = pt.copy()
			pt = alpha * np.dot(W, pt) + (1-alpha) * p0
		#print 'ppt:', ppt
		#print 'pt:', pt
		
		if np.mean(abs(pt - ppt)) < 0.0001:
			mean = np.mean(pt)
			err_truth = np.mean([abs(x-1) for x in pt])
			err_p1 = np.mean(abs(pt-p1))
		
			print str(i+1) + ', Alpha: ' + str(alpha) + ', Mean: ' + str(np.mean(pt)) + ', Err_GT: ' + str(err_truth) + ', Err_p1: ' + str(err_p1)
			f.write(str(i+1) + ', Alpha: ' + str(alpha) + ', Mean: ' + str(np.mean(pt)) + ', Err_GT: ' + str(err_truth) + ', Err_p1: ' + str(err_p1) + '\n')
		else:
