...
def kleinbergMatrix(M):
	W = np.zeros((len(M), len(M)))
...
def kleinberg(W, A, nodes, fileName):
	savingPath = EXPERIMENT_PATH + OUTCOME_PATH + 'Kleinberg_Iterative/'
...
def main():
	...
	for graphFileName in csvFiles:
		...
		(W, A) = kleinbergMatrix(M)
		kleinberg(W, A, nodes, fileName)
